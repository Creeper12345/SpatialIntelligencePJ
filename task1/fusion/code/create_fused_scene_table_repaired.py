...
def main():
    ...
    parser.add_argument("--out-dir", default="output/garden_fused_table_repaired")
    ...
    parser.add_argument("--object-b-obj", default="task1/threestudio_outputs/task1_text_b/dreamfusion_sd_basketball_6000@20260531-151908/save/it6000-export/basketball_mesh_t10.obj")
    parser.add_argument("--object-c-obj", default="task1/threestudio_outputs/task1_zero123/stable_lowmem@20260531-123503/save/it600-export/zero123_object_mesh_t10.obj")
    ...
    # The previous automatic placement used a low ground percentile, which put the
    # objects below the visible tabletop. In the garden reconstruction, the central
    # table surface appears as a dense horizontal layer around y ~= 2.1.
    table_region = (
        (scene_xyz[:, 0] > -2.0) & (scene_xyz[:, 0] < 2.0) &
        (scene_xyz[:, 2] > 0.0) & (scene_xyz[:, 2] < 3.0) &
        (scene_xyz[:, 1] > 1.7) & (scene_xyz[:, 1] < 2.4)
    )
    if int(table_region.sum()) > 1000:
        table_y = float(np.median(scene_xyz[table_region, 1]))
    else:
        table_y = 2.12
    table_z = float(np.median(scene_xyz[table_region, 2])) if int(table_region.sum()) > 1000 else 1.25
    table_z = float(np.clip(table_z, 0.8, 1.8))
    placements = {
        "A_video_3dgs": [-1.45, table_y + 0.03, table_z],
        "B_basketball": [0.0, table_y + 0.03, table_z + 0.12],
        "C_zero123": [1.45, table_y + 0.03, table_z],
    }

    # Tabletop-size objects: large enough to be visible in walkthrough renders,
    # but much smaller than the garden scene scale.
    obj_a = transform_gaussians(object_a, target_height=0.45, target_center=placements["A_video_3dgs"])

    pts_b, col_b = sample_mesh_points(args.object_b_obj, 320000, seed=11)
    pts_c, col_c = sample_mesh_points(args.object_c_obj, 260000, seed=17)
    obj_b = points_to_gaussians(pts_b, col_b, props, scene.dtype, 0.46, placements["B_basketball"], splat_radius=0.006, opacity=0.72)
    obj_c = points_to_gaussians(pts_c, col_c, props, scene.dtype, 0.50, placements["C_zero123"], splat_radius=0.006, opacity=0.72)
