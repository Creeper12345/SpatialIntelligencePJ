...
def main():
    ...
    parser.add_argument("--out-dir", default="output/garden_fused_table")
    ...
    obj_a = transform_gaussians(object_a, target_height=0.70, target_center=placements["A_video_3dgs"])

    pts_b, col_b = sample_mesh_points(args.object_b_obj, 160000, seed=11)
    pts_c, col_c = sample_mesh_points(args.object_c_obj, 110000, seed=17)
    obj_b = points_to_gaussians(pts_b, col_b, props, scene.dtype, 0.60, placements["B_basketball"], splat_radius=0.018)
    obj_c = points_to_gaussians(pts_c, col_c, props, scene.dtype, 0.65, placements["C_zero123"], splat_radius=0.018)
