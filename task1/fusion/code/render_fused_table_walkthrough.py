...
def main():
    ...
    ap.add_argument('--out-dir', default='output/garden_fused_table/walkthrough')
    ...
    target = centers.mean(axis=0) + np.array([0.0, 0.45, 0.0], dtype=np.float32)
    radius = 4.4
    y_base = float(target[1] + 1.15)

    video_path = out_dir / 'task1_scene_fusion_table_walkthrough.mp4'
    ...
    for i in range(args.frames):
        ...
        frame = render_points(points, colors, eye, target, args.width, args.height, fov_deg=43.0)
    ...
    sheet.save(out_dir / 'task1_scene_fusion_table_preview.png')

    print(f'points_rendered: {len(points)}')
    print(f'video: {video_path}')
    print(f'preview: {out_dir / "task1_scene_fusion_table_preview.png"}')
